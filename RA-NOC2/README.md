...
The possible experiment names are: threads, read_prop, value_size, txn_size, num_servers, distribution, num_key, freshness.
...
To generate a given plot from the paper therefore run the corresponding experiment for example number of client threads, then process its results, the run plot on the processed file, and identify among the generated plots the one you are looking for. Specifically, you may use this procedure to reproduce the results seen in Figure 6.
